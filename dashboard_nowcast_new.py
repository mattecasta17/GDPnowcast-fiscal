...
#------------------------------------------------- Config
st.set_page_config(page_title="GDP Nowcast Dashboard", layout="wide")

NOWCAST_DIR = "nowcast_Q"
METRICS_DIR = "metrics_Q"

#------------------------------------------------- Detect files
quarters = sorted([
    f.replace("nowcast_", "").replace(".csv", "")
...
#------------------------------------------------- Sidebar
st.sidebar.title("Nowcast Dashboard")
...
if view_mode == "Single Quarter":
    ...
    nowcast_file = os.path.join(NOWCAST_DIR, f"nowcast_{selected_q}.csv")
    metrics_file = os.path.join(METRICS_DIR, f"metrics_{selected_q}.csv")
    ...
    news_file = f"news_Q/news_{selected_q}.xlsx"
    ...
    # Read sheets in the same order as in Excel
    xls = pd.ExcelFile(news_file)
    sheet_order = xls.sheet_names  # vintage order as in Excel

    # Build df_impacts preserving the sheet order
    impacts = []
    for i, sh in enumerate(sheet_order):
        ...
        df_news["VintageIdx"] = i  # index for sorting
        impacts.append(df_news)
    ...
    # Ensure categorical ordering on the x-axis matches the sheet order
    ...
    fig = px.bar(
        ...
        title="Weekly Impact by Category",
    )
...
#================================================= Multi Quarter
    ...
    for q in quarters_to_plot:
        nowcast_file = os.path.join(NOWCAST_DIR, f"nowcast_{q}.csv")
        metrics_file = os.path.join(METRICS_DIR, f"metrics_{q}.csv")
    ...
    def last_friday(year, month):
        # take last day of the month and go back until Friday
    ...
    for q in quarters_to_plot:
        df_q = pd.read_csv(os.path.join(NOWCAST_DIR, f"nowcast_{q}.csv"))
        ...
        # continuous line of nowcast
        ax.plot(df_q["date"], df_q["y_new"], label=q)

        # square for actual value (advance release = last obs)
    ...
    # x-axis always from Nov t-1 to Jan t+1
    ...
    # 1) Loop only through quarters of the selected year
    for q in quarters_to_plot:
        news_file = f"news_Q/news_{q}.xlsx"
        ...
        if impacts:
            ...
            # 2) Sum impact for each category within the quarter
            df_q_agg = (
    ...
    # 3) Combine all quarters of the selected year
    ...
    # Select last observation of each quarter
