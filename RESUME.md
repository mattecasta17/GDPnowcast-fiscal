...
**Last touched:** 2026-06-01 (**Phase 3a GATE PASSED** — v1 DFM estimator + 2017q1 nowcast goldens frozen and validated bit-for-bit against v1 production; 3b port next — see Phase 3a block)
...
**▶ NEXT: Phase 3b** (de-MATLAB port into `src/gdpnowcast/`: `dfm_spec`, `transform`, `_dfm_support`, `dfm`, `news`, `decomposition`, `nowcast/runner`, each with golden parity at `1e-6`; fix v1 bugs; remove `+366`; one parameterised runner). Note: the ported `News_DFM`/runner must skip the **same 3** vintages to match the nowcast golden (Phase-4 candidate fix: handle the no-news/observed cases instead of crashing). The data layer is done and verified — Phase 3 parity runs on `data/US_new_v1` (v1 panel) to isolate port-correctness from the Phase-2 data-panel difference (a Phase-4 decision).
...
**Current branch tip:** `refactor/v2` — Phase 2 pushed, then Phase 3 plan committed (`4eafca2`), then **Phase 3a executed inline** (LOCAL, not yet pushed): `28161cc` ruff force-exclude → `bd1a77f` pre-commit end-of-file-fixer exclude → `2406def` NumPy-2 shims → `8f79582` freeze harness → `11c55c8` estimator golden → `1014118` nowcast golden. `main` untouched at `d50a2bc`, never merged into. `data/` holds fetched vintages (`US_new`/`US_fiscal` 486 each) + v1 backups (`US_new_v1` 486, `US_fiscal_v1` 485) — all gitignored.

**⏸ PAUSED 2026-06-01 — resume from A4.** This RESUME edit is the **only uncommitted change** (besides untracked `.claude/`); it is intentionally left uncommitted. **Tomorrow, restart at A4:** (1) `git add RESUME.md && git commit -m "docs(phase3a): record golden-baseline gate + pickle agreement"`; (2) optionally `git push origin refactor/v2` to back up the 6 local 3a commits; (3) then begin **Phase 3b** (B1 `dfm_spec` → B2 `transform` → B3 `_dfm_support` → B4 `dfm` → B5 `news` → B6 `decomposition` → B7 `runner` → B8 mypy/CI), each with golden parity at the tight default `rtol=atol=1e-6` (A2's `pickle_max_abs_dC ≈ 5e-15` justified keeping it tight). The ported `News_DFM`/runner must skip the **same 3** vintages (2016-12-30, 2017-02-24, 2017-04-28) to match `dfm_legacy_nowcast.json`.
