...
## 🟢 Phase 4 body — STEP 1: the off-grid `(advance−1)` headline step (DESIGN DONE, ⏸️ awaiting spec approval)

The gate is closed; this is the FIRST implementation step of the Phase-4 body. **Brainstorming → spec → review cycle complete (2026-06-04); NOT yet implemented.** Spec: `docs/plans/2026-06-04-phase4-advance-minus-1-headline-design.md`.
...
- **▶ NEXT:** Matteo reviews the spec → on approval invoke `superpowers:writing-plans` for the implementation plan, then TDD-implement (build tool first to produce the `2017-04-27` vintage, then the failing test, then the helper + runner wiring). Then the rest of the Phase-4 body (other `CONFIG_*` + fiscal + full 2017-2025 backtest + `pca(GDPC1)` test + the MTSDS133FMS→`ch1` / DM quarterly+HLN methodology fixes).
